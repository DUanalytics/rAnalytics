...
barplot(counts, beside=T, legend=T, main="Car Distribution", xlab="Number of Gears",   args.legend = list(x='right',bty='n'))
...
palette('default')
palette(gray(seq(0.5,1,len=3)))
barplot(counts, beside=T, legend=T, col=palette(),  args.legend = list(x='right',bty='n'))
#Other Method
barplot(counts, beside=T, col=palette())
legend(x= 'topright',legend = rownames(counts), fill=palette(),    title='No of Gears')
?legend
...
barplot(counts, beside=T, col=palette())
legend(x= 2, y=15,legend = rownames(counts), fill=palette(),  title='No of Gears')
